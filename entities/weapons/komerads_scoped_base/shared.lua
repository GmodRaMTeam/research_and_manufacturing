...
function SWEP:IronSight()
    ...
    if self.Owner:KeyPressed(IN_SPEED) and not (self.Weapon:GetNWBool("Reloading")) then
        -- If you hold E and you can shoot then
        if self.Weapon:GetNextPrimaryFire() <= (CurTime() + 0.3) then
            self.Weapon:SetNextPrimaryFire(CurTime() + 0.3)                -- Make it so you can't shoot for another quarter second
        end
        self.IronSightsPos = self.RunSightsPos                    -- Hold it down
    ...
    if self.Owner:KeyDown(IN_SPEED) and not (self.Weapon:GetNWBool("Reloading")) then
        -- If you hold E or run then
        if self.Weapon:GetNextPrimaryFire() <= (CurTime() + 0.3) then
            self.Weapon:SetNextPrimaryFire(CurTime() + 0.3)                -- Make it so you can't shoot for another quarter second
        end -- Lower the gun
    end

    if self.Owner:KeyReleased(IN_USE) or self.Owner:KeyReleased(IN_SPEED) then
        -- If you release E then
        self:SetIronsights(false, self.Owner)                    -- Set the ironsight true
        self.DrawCrosshair = self.XHair
    end

    if self.Owner:KeyPressed(IN_SPEED) or self.Owner:KeyPressed(IN_USE) then
        -- If you run then
        self.Owner:SetFOV(0, 0.2)
        self.DrawCrosshair = false
        if CLIENT then
    ...
    if self.Owner:KeyPressed(IN_ATTACK2) and not self.Owner:KeyDown(IN_SPEED) and not (self.Weapon:GetNWBool("Reloading")) then
        self.Owner:SetFOV(75 / self.Secondary.ScopeZoom, 0.15)
        self.IronSightsPos = self.SightsPos                    -- Bring it up
        self.IronSightsAng = self.SightsAng                    -- Bring it up
        self.DrawCrosshair = false
    ...
    if (self.Owner:KeyReleased(IN_ATTACK2) or self.Owner:KeyDown(IN_SPEED)) and not self.Owner:KeyDown(IN_USE) and not self.Owner:KeyDown(IN_SPEED) then
        self.Owner:SetFOV(0, 0.2)
...
function SWEP:DrawHUD()
    ...
    if self.Owner:KeyDown(IN_ATTACK2) and (self:GetIronsights() == true) and (not self.Owner:KeyDown(IN_SPEED) and not self.Owner:KeyDown(IN_USE)) then
