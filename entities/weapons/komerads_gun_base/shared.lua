...
function SWEP:RicochetCallback(bouncenum, attacker, tr, dmginfo)
    ...
    if (bouncenum > self.MaxRicochet) then
        return
    end
...
function SWEP:BulletPenetrate(bouncenum, attacker, tr, paininfo)
    ...
    if (trace.StartSolid or trace.Fraction >= 1.0 or tr.Fraction <= 0.0) then
        return false
    end
...
function SWEP:Silencer()
    ...
    if self.NextSilence > CurTime() then
        return
    end
