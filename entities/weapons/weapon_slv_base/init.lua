...
function SWEP:CreateDecal(tr)
	...
	if not sDecal then return end
	util.Decal(sDecal, tr.HitPos +tr.HitNormal, tr.HitPos -tr.HitNormal)
end

function SWEP:PlayThirdPersonAnim(anim)
	anim = anim or PLAYER_ATTACK1
	if not game.SinglePlayer() or self.Owner:IsNPC() then self.Owner:SetAnimation(anim); return end
...
function SWEP:PlaySound(sSound, flDelay, bClientOnly)
	...
	if not self.tblSounds[sSound] and not iWav then return end
...
function SWEP:OnPickedUp(entOwner)
	...
	if not entOwner:IsPlayer() then return end
	if self.Primary.Ammo ~= "none" and not util.IsDefaultAmmoType(self.Primary.Ammo) then
		...
		if iAmmoPickup > 0 then
			...
			if IsValid(entWeapon) and entWeapon:GetClass() == self:GetClass() and not entWeapon.Primary.SingleClip and entWeapon:Clip1() == 0 and entWeapon:CanReload() then
				entWeapon:Reload(true)
			end
			if self:Clip1() == 0 and not self.Weapon.Primary.SingleClip and not self.Weapon.delayReloaded then self:Reload(true); return end
	...
	if self.Secondary.Ammo == "none" or self.Secondary.Ammo == self.Primary.Ammo then return end
