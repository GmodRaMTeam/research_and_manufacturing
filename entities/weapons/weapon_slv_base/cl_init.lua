...
function SWEP:CustomAmmoDisplay()
	...
	if not self.Primary.SingleClip then
...
usermessage.Hook("HLR_HUDItemPickedUp", function(um)
	if LocalPlayer() == nil or not LocalPlayer():Alive() then return end
...
usermessage.Hook("HLR_SWEPDoReload", function(um)
	...
	if not IsValid(wep) or LocalPlayer():GetActiveWeapon() ~= wep then return end
	wep.Weapon:Reload()
end)

usermessage.Hook("slv_tpanim", function(um)
	local ent = LocalPlayer():GetActiveWeapon()
	if not IsValid(ent) or not IsValid(ent.Owner) then return end
	local anim = um:ReadShort()
	if not anim then return end
	GAMEMODE:DoAnimationEvent(ent.Owner, anim == PLAYER_RELOAD and PLAYERANIMEVENT_RELOAD or PLAYERANIMEVENT_ATTACK_PRIMARY)
end)
