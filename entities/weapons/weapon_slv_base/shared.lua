...
function SWEP:GetAmmoPrimary()
	if not self.Weapon.Owner:IsPlayer() then return 9999 end
...
function SWEP:PrimaryAttack(ShootPos, ShootDir)
	if (not self.Weapon.InWater and self.Owner:WaterLevel() == 3) then return end
...
function SWEP:Attack(flCone, ShootPos, ShootDir)
	if not self:CanPrimaryAttack() then if self.Weapon.AutoReload then self:Reload() end; return end
	if SERVER then
...
function SWEP:CanPrimaryAttack()
	if self:Clip1() == 0 then if SERVER and self.Weapon.EmptySound then self.Weapon:EmitSound("Weapon_Pistol.Empty") end; return false end
	return true
end
...
function SWEP:CanReload()
	return not self.Primary.SingleClip and self:GetAmmoPrimary() > 0 and self:Clip1() < self.Primary.DefaultClip and not self.Weapon.delayReloaded and not self.Weapon.nextReload
end
...
function SWEP:Think()
	if self.attackDelay and CurTime() >= self.attackDelay then self:DoAttack(); self.attackDelay = nil end
	if self.Weapon.nextIdle and CurTime() >= self.Weapon.nextIdle then
		if (self.Weapon.AutoReload or self.bReloadOnNextIdle) and self:Clip1() == 0 and self.Weapon:CanReload() then self.bReloadOnNextIdle = nil; self:Reload(); return end
	...
	if self.Weapon.SingleReload and self.Weapon.nextReload and CurTime() >= self.Weapon.nextReload then
	...
	if self.Weapon.delayReloaded and CurTime() >= self.Weapon.delayReloaded then
		local iClip = self:Clip1()
		local iClipMax = self.Primary.DefaultClip
		local iClipAdd
		if not self.Weapon.SingleReload then iClipAdd = iClipMax -iClip
		else iClipAdd = 1 end
		...
		if self.Weapon.nextReload and ((iClip +iClipAdd) == iClipMax or self:GetAmmoPrimary() == 0) then
...
function SWEP:ShootEffects(bSecondary)
	...
	local act
	if not bSecondary then act = ACT_VM_PRIMARYATTACK
...
function SWEP:EntsInViewCone(pos, flDistance)
	for k, v in pairs(ents.FindInSphere(pos, flDistance)) do
		if IsValid(v) and v ~= self and v ~= self.Owner and (v:IsPhysicsEntity() or v:IsNPC() or v:IsPlayer()) and self.Owner:EntInViewCone(v,25) then
			return true
		end
	end
	return false
...
function SWEP:DoMeleeDamage(iDist, iDmg)
	...
	local act
	if tr.Hit or bEntsInViewCone then
		if IsValid(tr.Entity) or bEntsInViewCone then
